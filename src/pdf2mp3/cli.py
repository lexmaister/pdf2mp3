...
def get_args() -> argparse.Namespace:
    """Init arguments parser for the app"""
    ...
    runtime_group.add_argument(
        "--overwrite", action="store_true", help="Replace an existing OUTPUT_MP3."
    )
    runtime_group.add_argument(
        "--no-progress",
        action="store_false",
    ...
    if args.output_mp3 is None:
        args.output_mp3 = Path.cwd() / (args.input_pdf.stem + ".mp3")
    ...
    if not (0.0 <= args.compression <= 1.0):
        parser.error(
            f"Compression level must be between 0.0 and 1.0. Got: {args.compression}"
        )
